...
describe('rollFlux', () => {
  it('triggers when the rng rolls below the flux chance', () => {
    expect(rollFlux({ random: () => 0.1 })).toBe(true);
    expect(rollFlux({ random: () => 0.9 })).toBe(false);
  });
  it('accepts a null rng (falls back to Math.random)', () => {
    expect(typeof rollFlux(null)).toBe('boolean');
  });
});
