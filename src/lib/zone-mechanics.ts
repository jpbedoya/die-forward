...
/**
 * The combat mechanic for each zone. Mirrors the `meta.mechanic` field in the
 * zone JSON — kept as a static map so this module has no zone-loader / JSON
 * dependency and can be shared verbatim with the web/agent side.
 */
const ZONE_MECHANICS: Record<string, ZoneMechanic> = {
  'sunken-crypt': 'NONE',
  'ashen-crypts': 'BURN',
  'frozen-gallery': 'CHILL',
  'living-tomb': 'INFECTION',
  'void-beyond': 'FLUX',
};

/** The mechanic for a zone. NONE if the zone is unknown or has no mechanic. */
export function getZoneMechanic(zoneId: string): ZoneMechanic {
  return ZONE_MECHANICS[zoneId] ?? 'NONE';
}
...
export function rollFlux(rng: { random(): number } | null): boolean {
