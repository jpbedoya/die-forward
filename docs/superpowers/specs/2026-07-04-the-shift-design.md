...
**🔴 LAUNCH-BLOCKING SECURITY GATE (flagged by automated review during 3b, July 2026): unauthenticated balance mutation / IDOR.** The entire session API has *no request authentication* — every route (start/advance/death/victory) derives the acting Player from a body-supplied `authId`/`walletAddress`, unverified. Phase 3b's coin staking makes this the first path that *mutates a balance*: an attacker who knows a victim's (public, on-chain) wallet address can POST coins-mode `start` calls as them and force-deduct their Pale Coins (grief only — coins are earned-only/non-transferable, so no value theft). This is **acceptable on devnet with valueless coins** but **must be fixed before coins gain any worth, any transfer path, or mainnet**. The fix is the A1 carve-out done properly: authenticate the request (session token verification or a signed wallet challenge), derive the Player from the authenticated principal server-side, and reject any body-asserted identity that doesn't match — applied across ALL player-mutating routes, not just start. Tracked as its own pre-launch security phase; deliberately NOT built in 3b (app-wide auth is far larger than the coin economy). **The same unauthenticated-write surface extends to the InstantDB namespaces themselves (partially closed, phase 3b final review July 2026):** `instant.perms.ts` previously listed rules only for `players`/`deaths`/`corpses`/`settings`, leaving the *live* money namespaces `gameSettings` (holds `coinPool`/`coinBonusPercent`/`victoryBonusPercent`), `runReceipts`, and `sessions` at InstantDB's **default-allow** — any authenticated client could set `coinPool` arbitrarily large (mint), forge run receipts, or tamper with live session state directly, bypassing the server routes entirely. These three now carry **deny-by-default** perms (view-only; server admin client bypasses perms for legitimate writes). **Follow-up required:** the `/admin` page currently writes `gameSettings` from an *unauthenticated* browser InstantDB client (client-side `ADMIN_WALLETS` gate only) — i.e. it rode the same default-allow hole — so under the new deny it must move its writes to an authenticated admin server route (signed admin-wallet challenge → admin `db` write) before admin settings editing works again. That migration is a separate, non-money change and is not part of this money-integrity fix.
