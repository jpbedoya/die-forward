...
**(done, phase 3a July 2026)** — client seeded layer shipped: `lib/world-shift.ts` (`utcDayKey` + `getDailyShift`, modifier pool of 2-3, side-door seals, validated edge mask, degradation ladder), threaded through `generateDungeonGraph(zoneId, rng, shift?)` and `startGame(..., chosenModifierId?)` with draw stability; `dailyShiftEnabled` typed setting (default true), admin-toggleable at `/admin`. Deterministic per (zone, day), fully client-side. **Remaining for 3b:** the server-authority carve-out above (A1: VRF-committed `runSeed` + server-stamped date for staked runs) and staked-run date stamping are not yet built — today's shift is unauthenticated client compute for every rung, including Blood-Bound. **3b receipt requirement (from the 3a final review):** run reproducibility now needs the full tuple `(runSeed, dayKey, dailyShiftEnabled, chosenModifierId)` — all four are memory-only today; the 3b run receipt must capture them or shifted runs cannot be replayed/verified.

**✅ RESOLVED (July 2026) — was: 🔴 LAUNCH-BLOCKING SECURITY GATE (flagged by automated review during 3b): unauthenticated balance mutation / IDOR.** History of the gate, for context: the entire session API had *no request authentication* — every route (start/advance/death/victory) derived the acting Player from a body-supplied `authId`/`walletAddress`, unverified. Phase 3b's coin staking made this the first path that *mutates a balance*: an attacker who knew a victim's (public, on-chain) wallet address could POST coins-mode `start` calls as them and force-deduct their Pale Coins (grief only — coins are earned-only/non-transferable, so no value theft). This was **acceptable on devnet with valueless coins** but **had to be fixed before coins gained any worth, any transfer path, or mainnet**. The same unauthenticated-write surface extended to the InstantDB namespaces themselves: `instant.perms.ts` previously listed rules only for `players`/`deaths`/`corpses`/`settings`, leaving the *live* money namespaces `gameSettings` (holds `coinPool`/`coinBonusPercent`/`victoryBonusPercent`), `runReceipts`, and `sessions` at InstantDB's **default-allow** — any authenticated client could set `coinPool` arbitrarily large (mint), forge run receipts, or tamper with live session state directly, bypassing the server routes entirely. These three now carry **deny-by-default** perms (view-only; server admin client bypasses perms for legitimate writes).

**What shipped (dedicated pre-launch security phase, July 2026):** requests now carry an `Authorization: Bearer <customToken>` header — the InstantDB customToken the client already holds (minted client-side, verified server-side via `db.auth.verifyToken`, fail-closed on any missing/invalid/expired token). `src/lib/auth-server.ts` provides `verifyAuthToken(req)` (returns a verified `AuthedIdentity` or null), `deriveAuthIdFromEmail`, and `isAdminAuthId`. `session/start` in **coin mode now REQUIRES a verified token**: the acting Player is looked up SOLELY by `identity.authId`; the body's `authId`/`walletAddress` never reaches the balance op — this closes the IDOR root directly. SOL/free-mode `start` keeps working when unauthenticated (verified identity overrides the body when present; body `authId`/`walletAddress` remains an untrusted fallback otherwise, firewalled from leaderboard/currency writes as before). `death`/`victory`/`advance` add cross-account defense-in-depth: a *valid* token whose `authId` disagrees with the session's own `authId` is rejected (`sessionAuthMismatch`); the secret session token remains the sole gate when no token is presented (no false 403s on pre-hardening sessions). **Coin *earn* and every player-row write in `death`/`victory` are additionally gated on token-verified provenance** — `start` stamps `authVerified` on the session (`true` for coin-mode and any token-verified run, `false` for a body-supplied free/sol fallback), and the single `tx.players` write in each route fires only when `authVerified` holds. This closes the *earn*-side counterpart to the deduct IDOR the automated review surfaced: a no-token run can no longer credit `paleCoins`/stats to a body-asserted account (a scriptable, unbounded self-mint / cross-account write before the gate). Death/corpse/runReceipt content still records for every run; coin-mode stake returns are unaffected (`|| stakeMode === 'coins'` keeps them, and coins were provably verified at start). `session/cleanup` (money-settling) now requires `CRON_SECRET` via `Authorization: Bearer` or `x-cron-secret`, open-and-warned if the env var is unset. The `/admin` page's write path moved to an authenticated admin server route (`/api/admin/settings`, plus admin/bestiary and admin/content), gated by `verifyAuthToken` + `isAdminAuthId`; `coinPool` is excluded from the writable settings whitelist so an admin session cannot mint the pool via the settings editor.

**Residuals (accepted; (a)–(g) are non-balance-mutating or authenticated-admin self-harm; (h) is the one open cross-account balance write, but it requires authentication and is grief-only on devnet — launch-blocking before coins gain value):**
- **(a) Offline-local guests** (`guest-offline-*`) hold no token and stay fully client-authoritative by design — firewalled from leaderboard/coin writes exactly as before (see A1 offline firewall, §9.0 done-note).
- **(b) Agent routes** (`agent/action`, `agent/state`) are keyed off the raw `sessionId` rather than a verified identity — a weaker gate. Accepted because agent runs grant no coins/stats (receipt-only, `authId: null`, `coinDelta: 0` — see §9.0 done-note); a leaked/guessed `sessionId` can only perturb an agent run (grief), never move money. The external harness contract (raw `sessionId`) is unchanged.
- **(c) SOL payout address** is still body-supplied on `session/start`; accepted because the on-chain stake transaction itself binds the payout address — this is escrow's concern, not the session API's.
- **(d) Login nonce replay window:** `src/app/api/auth/wallet/route.ts` accepts a nonce within a ±5-minute freshness window and has no server-side replay store, so a captured fresh signature is technically replayable within that window. Low risk (requires intercepting a live signed message), pre-existing, not introduced by this phase.
- **(e) LAUNCH STEP — not yet done:** `CRON_SECRET` must be set in the Vercel production env before launch; until then `session/cleanup` is open (warned-but-allowed).
- **(f) Admin settings** have a key-whitelist but no value-range validation — accepted because the surface is admin-gated (trusted-admin self-harm only, not an external attack surface).
- **(g) `src/app/admin/page.tsx`** still writes InstantDB `tx.playlists` directly from the browser client (unauthenticated InstantDB write, gated only by client-side `ADMIN_WALLETS` check) — a separate, non-money, unguarded surface tracked as a follow-up ticket, not part of this fix.
- **(h) `players` InstantDB perms lack an ownership check** — `players.allow.update` is `"auth.id != null"`, so any *authenticated* InstantDB client can `tx.players[victimId].update({ paleCoins: … })` directly: a cross-account balance/stat/streak write that bypasses every server route (the perms-layer analogue of the IDOR the routes now close on both the deduct and earn sides). Unlike (a)–(g) this **is** balance-mutating, but by an authenticated party, and grief-only on devnet with valueless, non-transferable coins. It cannot be closed by an ownership rule today because InstantDB's `auth.id` is an internal UUID unequal to our `authId` (wallet address / guestId — see the note in `instant.perms.ts`); the fix requires a schema change (store the InstantDB user id on the player row and match `data.<field> == auth.id`). **Launch-blocking before coins gain worth, any transfer path, or mainnet** — tracked alongside A1's server-authority carve-out.
...
**UGC moderation gate (A2 — ships before any rebroadcast feature):** any player-authored text surfaced to *other* players (Echo Husk phrases, Architect-wall names/final words, corpse final words in shared surfaces) passes profanity/URL filtering, carries a report button, and is trust-weighted (account age / staked history). No filter, no rebroadcast — this is a store-review-level requirement, not polish. **(done, phase 4b July 2026)** — see the phase 4b done-note below.

**(done, phase 4a July 2026)** — the non-UGC slice of the community layer shipped end to end: a server-receipted nightly aggregation cron (`/api/game/shift`) writes a deny-by-default `worldShifts` namespace in InstantDB; A5 integrity is enforced via distinct-account keying (`authId ?? walletAddress`), a per-account death cap, a trailing-24h + future-clamp window, and receipts-only ingestion (`src/lib/world-shift-agg.ts`, `src/app/api/game/shift/route.ts`). The apex threat gets both halves of its treatment: a tested +15% HP/damage buff (`applyApexBuff` in `combat-math.ts`) and a bounty (seeded bonus loot roll + extra bestiary mastery credit on kill). Curse and Architect marks are non-UGC — a visible statue/wall flag on the node, no player text — merged additively into the seeded map layer client-side (`fetchCommunityShift`/`mergeShift` in `lib/world-shift.ts`), and the merge degrades gracefully to the seeded-only layer on any fetch failure or offline session. The markers are surfaced passively in `mobile/app/play.tsx` (apex near the enemy card, cursed/architect in the node header) via `community.apex`/`community.cursed`/`community.architect`, identical across all 7 locale catalogs.

**(done, phase 4b July 2026)** — the A2 moderation gate shipped, and with it Echo Husk phrases + Architect-wall UGC. Server-authoritative: `src/lib/moderation.ts` is a self-contained profanity/URL filter — NFKC normalization, Cyrillic/Greek homoglyph folding, leet-substitution folding, and a generic-TLD/protocol/`@handle` URL blocker — plus trust-weighting (`isTrustedAuthor`: account age ≥3d default, staked history, wallet-auth; fail-closed on unknown/guest authors) and report-count suppression (`buildSuppressedSet`, backed by the new `reports` namespace and an authenticated `/api/moderation/report` route that looks up the reported text/author server-side — from the death or corpse row — and never trusts client-supplied text, counting distinct reporters). Rebroadcast text is sourced **only** from `runReceipts.finalMessage` (server-trusted, added in Task 1 — the aggregation never reads raw `deaths` rows for text), moderated inside `runAggregation`/`selectModeratedUGC`, and written as `echoPhrases`/`architectEntries` on `worldShifts`. Echo Husks' *Repeating* signature recites a moderated phrase in combat (`'repeating'` in `creature-rules.ts`); Architect walls inscribe moderated `@nickname: words` at the deadliest node; a report button lives on the corpse surface (`mobile/lib/api.ts` → `/api/moderation/report`). **Display surfaces are also filtered client-side:** every place another player's raw `finalMessage` is shown to a viewer — discovered corpses (`play.tsx`), the home death feed (`index.tsx`), and the feed screen (`feed.tsx`) — passes through a mobile mirror of the content filter (`mobile/lib/moderation.ts` `containsBlockedContent`), replacing blocked text with `t('corpse.redacted')`. So blocked content is suppressed on both the rebroadcast (Echo/Architect) channels AND the direct-display shared surfaces, closing §E.1's "corpse final words in shared surfaces" requirement.

**Deferred/residual, carried into phase 4b or later:**
1. **A5 sybil residual:** distinct-account keying falls back to `walletAddress` when `authId` is absent, and wallets are cheap to mint — on devnet, ~4 wallets defeats `curseNodeThreshold`. This is spec-compliant for the devnet milestone but must be hardened before mainnet (prefer `authId`-only keying, or gate aggregation on a staked/verified account).
2. ~~Architect corpse-wall UGC (names/final words) and Echo Husk phrases~~ — **shipped, see the phase 4b done-note above.**
3. **`curseNodeThreshold` and `apexMinKills` are not yet in the admin settings whitelist** — the aggregation cron reads them from `gameSettings` but falls back to hardcoded defaults (10 and 3 respectively) since neither key is exposed for admin tuning yet.
4. **DEFERRED out of phase 4b:** Echo Husk *Listening* combat mechanic (attacking on consecutive turns draws its focus — not implemented, only *Repeating* shipped); an admin moderation dashboard/appeal flow (reports are recorded and suppress rebroadcast, but there's no review UI); a richer profanity model — the baseline wordlist is a small, guessable substring list, mitigated (not solved) by the trust + report layers, a **documented residual**; an Architect-entry report button — `architectEntries` are `{name, words}` with no `deathId` FK back to a reportable row, so reporting ships on the corpse surface only; cross-script confusable folding beyond the common Cyrillic/Greek map (§ above) remains a baseline limit, not exhaustive Unicode confusable coverage.
...
- **3 depth tiers** (unchanged danger scaling), each tier a band of the map. **Every node carries an authored `depth` field** — tier comes from the node, never from traversal position. **(done, phase 2a July 2026)** — graph schema shipped as `ZoneNode`/`ZoneGraphLayout` + `validateZoneGraph` in `zone-loader.ts`; sunken-crypt ships a 21-node/26-edge two-lane graph with cross-links, identical across all 7 locale packs; other zones auto-linearize. Edges are constrained to descend exactly one depth.
- **Canonical depth projection (feasibility constraint, load-bearing):** `room` is a monotonic integer welded from the UI through server validation to the on-chain `u8` (`final_room`), and `highestRoom` drives zone unlocks + the leaderboard. The DAG must *project* to that integer: `highestRoom` is redefined as **max node-depth reached**, and `ProgressBar`/`maxRooms`/on-chain writes all consume the projection. The anchor program is not modified. Death/Corpse records gain a **node id** field; corpse adjacency re-keys on node/depth-tier instead of `room ± 1`. **(done, phase 2a July 2026)** — `generateDungeonGraph` (content.ts) + GameContext `graph`/`currentNodeId`/`path`; `currentRoom` is now the 1-based depth projection everywhere (server, on-chain u8, highestRoom, Death/Corpse.room); server victory `maxRooms` fallback unified to 13; corpse discovery re-keyed on zone id + optional `nodeId` (was a display-name mismatch that never matched cross-player), exact-node matches sort first, second writer (agent submit_death route) converged too — legacy display-name rows orphaned by design.
...
**Remaining for 2b:** bespoke signature-node prose (the ~12/zone authoring budget above) and daily edges (open/close subset per shift) are not yet built. **(done, phase 2b July 2026)** — side chambers + item gates shipped: `side`/`gate: { item, consumes }` on `ZoneNode`, client-only traversal (no server sync), gate UI (`[ASIDE]`/`[UNSEALED]`/`[SEALED]`), gate-item consumption on entry, Bone Dust reveals branch types pre-choice (consumed). All 5 zones now ship full branching graphs (sunken-crypt 23 nodes incl. 2 side; ashen-crypts 20 incl. 1; frozen-gallery 20 incl. 1; living-tomb 20 incl. 1 ungated; void-beyond 21 incl. 1 ungated at depth 9, deviant topology) — the "other zones auto-linearize" fallback above no longer applies.

### 4.2 Presentation

- Path choice happens at the end of each room in the existing choice-button UI — no live map screen required for v1.
- **Hint contract (F2 — binding):** every branch hint is **dual-signal** — one sensory clue in bible voice *plus* one legible risk/reward tag the player can read cold: *"Right — distant water. (A cache, but the air is wrong.)"* Flavor alone previews nothing and makes the choice a coin flip; a hint must let the player trade a known reward against a known risk. The stamina **peek** then has a real job: it upgrades a hint into a certainty. **(done, phase 2a July 2026)** — hint contract v1 shipped: sense line + `[DANGER]`-style legible tag, `hint.*` keys in all 7 locale catalogs (incl. new vi); peek reveals one edge's type.
- **Comparison surface (F1 — v1-BLOCKING, promoted):** a post-run **path trail** on the death/victory screen — nodes visited plus branches declined. This is the minimum surface that makes yesterday≠today *felt* ("that door was open yesterday"); without it the daily shift is real in data and invisible in play. Cheap: the traversal data already exists. A live ASCII map remains a later nice-to-have. **(done, phase 2a July 2026)** — path-trail block shipped on both death and victory screens, `trail.*` keys.
...
| Echo Husks | *Repeating*: recites real final words from recent deaths in this zone (community layer); *Listening*: attacking on consecutive turns draws its focus — **(done, phase 4b July 2026): signature is now `'repeating'` — moderated phrase recital shipped (`creature-rules.ts`); *Listening* is explicitly DEFERRED, not implemented** |
...
**(done, phase 4c July 2026)** — the shared `renderDispatch` pipeline now feeds all three surfaces: home panel and zone-select (as before) plus a once-daily push at local morning. **STRICT F7** is enforced in code: the hourly fan-out cron (`src/app/api/game/dispatch/route.ts`) sends a push ONLY when the flagship zone's day has an apex threat or cursed nodes — quiet/mask-only days send **zero** pushes (early return), a deliberately conservative reading of "cap, not floor." Delivery is ≤1/day per user via `Player.lastDispatchDayKey` stamping (selection logic `selectFanoutRecipients`/`renderPushText` in `src/lib/dispatch-fanout.ts`, fully unit-tested); the Cartographer's 3 rotating registers (apex / curse / quiet-fallback) live in `renderPushText`. Permission is diegetic and optional — the Cartographer's ask after first death (phase 4b) — and nothing is gated on it; deniers see the identical dispatch on the home screen. Schema: `Player.pushToken` / `Player.timezone` / `Player.notifOptIn` / `Player.lastDispatchDayKey`. Fan-out runs via `expo-server-sdk` (`Expo.isExpoPushToken` filter, chunked, best-effort per-chunk send — never throws the request). **DEFERRED:** the rare personal Architect-corpse push (post-4b/A2 moderation dependency); per-locale push body (English only for now — the in-app panel is fully localized). **Not verified in this phase:** the on-device push path requires a native EAS rebuild (`expo-notifications` cannot be jest-verified) plus a real device and Expo push credentials — only the pure selection/render logic is tested.
...
- **The Binding Streak (F6 — what makes coin loss sting):** consecutive Coin-Bound escapes build a visible **seal tier** on the player's feed badge and corpse; a Coin-Bound death resets it to zero. Coins alone re-earn in ~3 runs — refungible, no tension; the streak is non-refungible, social, and public. This is the emotion that actually rehearses Blood-Bound: losing the thing you were showing off. **Abandonment ruling (phase 3b final review, July 2026):** a Coin-Bound run *abandoned* mid-descent (force-quit → later swept by `/api/session/cleanup`) **resets the streak to zero, exactly as a death does.** Otherwise force-quitting on the brink of death would strictly dominate dying — same stake cost (the coins were deducted at run start and burn to the pool either way), but the streak survives — turning the whole "losing the thing you were showing off" mechanic into an opt-out. Force-quitting is not an escape hatch. (A run that *reached the exit* but was never claimed is a cleared-but-unclaimed **win**: its stake is returned, no bonus, and the streak is left untouched — see §9.0 cleanup note.)
...
**(done, phase 3b July 2026)** — the Coin-Bound rung shipped end to end: `src/lib/coins.ts` / `mobile/lib/coins.ts` (`COIN_STAKE_OPTIONS = [60, 120, 240]`, `computeCoinEarn` — concave depth income `floor(4·√min(depth,13))` plus flat clear/first-clear bonuses, `computeCoinStakeSettlement`, `validateCoinStakeRequest`, `classifyVictorySettlement`). Death burns the coin stake to the `coinPool` counter on the `gameSettings` row (A4); victory returns the stake plus a **pool-funded** bonus (`bonusPercent` from the new `coinBonusPercent` setting, default 50%, floored and capped by `poolAvailable` so the pool can never go negative — population-net-negative by construction, never minted). **The Binding Streak (F6)** shipped as `nextStreak`/`sealTier` (tiers 0/1/2/3 at streak 3/7/15) — a Coin-Bound clear increments the streak, a Coin-Bound death zeroes it; the tier surfaces as a feed badge (`feed.seal.1/.2/.3` glyphs) and on the stake/death/victory screens (`coin.streak`, `death.coin.*`, `victory.coin.*` i18n keys). **Coin-Bound at the Toll** shipped: the stake screen offers coin binding (`stake.coin.*` keys) alongside SOL, gated the same as any stake path. **Run receipts** shipped: `buildRunReceipt` (`src/lib/coins.ts`) writes the full `(runSeed, dayKey, dailyShiftEnabled, chosenModifierId)` tuple the 3a review flagged as missing (see §3.1 done-note), plus outcome/coinDelta/streakAfter, to a new `runReceipts` namespace on every death/victory — closing that reproducibility gap. Agent-driven runs emit a receipt but never a coin grant (documented, not a bug). **Offline leaderboard firewall (A1, partial)** shipped client-side: `GameContext.tsx` gates `updateHighestRoom`/`updatePlayerClearedZone`/`api.advanceRoom` behind `!api.isOfflineSession(sessionToken)`, so an offline (unauthenticated) run's progression never reaches `highestRoom` or `clearedZones`. **This is a client-side JS guard only** — `instant.perms.ts` still permits any authenticated write to `players`, so a modified client could bypass it; A1's full server-authority carve-out (VRF-committed seed + server-stamped date + receipt-gated writes) remains open. **The request-authentication gap underneath it is now closed** — see the ✅ RESOLVED note in the §3.1 done-note above (dedicated security phase, July 2026): request auth via verified InstantDB token, coin-mode start requires auth, cross-account session rejection, authenticated admin writes. Residuals documented there (offline guests, agent-route gating, SOL payout address, login nonce window, CRON_SECRET launch step, admin value validation, admin playlist writes, and the launch-blocking `players`-perms ownership rule).
>
> **What REMAINS (not built in 3b):**
> - ~~The launch-blocking auth/IDOR gate~~ — **RESOLVED**, see §3.1 done-note. A1's VRF-committed-seed/server-stamped-date half (below) is still open.
> - **Pale Coin GATE migration:** the Ferryman's crossing (`ashen-crypts.json`, node gate `{ item: "Pale Coin", consumes: true }`) still spends the *item* drop, not the new `paleCoins` currency balance. Deliberately deferred — the item gate shipped in phase 2b and works as designed; migrating it to spend currency touches zone content data (all 7 locale packs) and belongs with the NPC/community-layer phase (§3.2), not a docs-and-admin task.
> - **Full VRF commit-reveal** for staked run seeds (A1's other half) — phase 4/launch gate.
> - **Unbound receipt-gated community aggregation** (A5) — phase 4, depends on the community layer.
>
> **Known limitations (accepted for devnet, tune before launch):**
> - **TOCTOU races on `coinPool`/player balance:** settlement reads-then-writes both counters without a transaction-level lock (documented at `5986198`/`f8bb72e` in git history); acceptable at devnet scale, must be revisited before real economic stakes.
> - **Pure-death coin earn undershoots its own balance guard:** the depth-only term (`floor(4·√depth)`, no clear bonus) caps out well under half of the "~1/3 of a respectable stake per run" target this section sets — a run that dies mid-zone earns roughly half what the heuristic intends. Needs calibration (higher depth coefficient or a partial-progress bonus) before launch tuning locks in.
...
**(done, phase 3b July 2026)** — posture switch shipped: `stakingPosture: 'hidden' | 'ritual' | 'open'` lives as a raw string column on the `gameSettings` row (same defaulted-client-side pattern as `enabledZones`), admin-editable via a `SettingSelect` at `/admin` (default `ritual`, matching the table above). Mobile reads it (`mobile/lib/stake-posture.ts`) to gate the Blood-Bound stake UI on `stake.tsx`; Coin-Bound is unaffected by the posture value, per the ladder rule that the first two rungs are universal. Not yet built: any *automatic* region/store-build selection of posture (today it's one global admin-set value, not split by distribution surface as the table implies) — that's a manual admin operation, not code.
...
1. **Screen strings → catalog extraction pass.** A dedicated phase-2 task migrates the ~277 screen literals into `en.json` keys. Mechanical; converges because the surface is enumerable and new literals are banned. **(done, phase 2b July 2026)** — extraction shipped: 530 new keys across `codex.*`, `stake.*`, `leaderboard.*`, `index.*`, `feed.*`, `bestiary.*`, `zoneSelect.*`, `play.*`, `combat.ui.*`, `death.*`, `victory.*`; catalogs now 605 keys, structurally identical across all 7 locales. Only the music-test screen and `[DEVNET]` badges remain literal (dev-only surfaces, out of player-facing scope).
2. **Content packs → per-locale pack files.** Zone prose localizes as whole translated packs (`sunken-crypt.ja.json` …), with `zone-loader` selecting by locale and falling back to English per-pack. This preserves the fragment-assembly architecture, gives translators full narrative context, and keeps locale drops content-only. Key-referencing packs (re-architecting the content engine) was considered and rejected as invasive. **Consequence:** all phase-2 DAG node prose is authored in the pack format from day one, so it inherits localization for free. **(done, phase 2a July 2026)** — loader locale selection shipped (`ZONE_LOCALE_MAP`, per-zone English fallback); i18n catalogs now cover 7 locales (vi added as English placeholders pending translation — see `docs/localization/RETRY_MANIFEST.md`).
...
| 2 | Heartstone / Bone Dust / Pale Coin / Eye of the Hollow are flavor-only stubs | **[this project]** §5 real effects; Pale Coin → currency (§9) — **partial (phase 1, July 2026):** Heartstone effect live (one-hit +10% defense warning across all four damage paths); Bone Dust, Pale Coin, and Eye of the Hollow map-related effects deferred to phase 2 — **done (phase 2b July 2026):** Bone Dust wired — reveals branch/gate types before choice, consumed on use (`revealPaths` in `getItemEffects`); Pale Coin is a functioning gate item (side-chamber gate, consumed) |
...
| 11 | Weapon type identities (reach/fast/heavy/lifesteal) are flat stat sticks | **[this project — partial]** synergies restore some identity; full weapon-type combat differences are **[follow-up]** — **partial (phase 1, July 2026):** 8 synergies live. (phase 2b July 2026: Bait shipped as a universal combat verb, not a weapon-specific one — weapon-type differentiation itself remains open, still [follow-up].) |
...
- 🟡 **A1 — Server-authority carve-out (supersedes parts of §3.1):** *staked runs are never offline.* Blood-Bound and Coin-Bound runs get a server/VRF-committed `runSeed` and server-stamped date at stake time (MagicBlock VRF exists for exactly this). Unbound stays client-side/offline — but is firewalled from leaderboards, currency grants, and community aggregation unless carrying a server run receipt. **Partial, phase 3b July 2026:** the offline-run firewall (leaderboard/currency writes gated on `!isOfflineSession`) and the run-receipt tuple both shipped — see §9.0 done-note. **The underlying request-authentication gap is now RESOLVED** (dedicated pre-launch security phase, July 2026 — see the ✅ RESOLVED note in the §3.1 done-note: verified InstantDB token auth, coin-mode start requires auth, cross-account session rejection, authenticated admin writes; residuals documented there). Still open for A1 specifically: the VRF-committed seed/server-stamped date for staked runs (full commit-reveal) — phase 4/launch gate.
- ✅ **A2 — UGC moderation before re-broadcast (blocks §3.2 Echo Husks/Architect walls):** any player-authored text (final words, nicknames) surfaced to *other* players passes profanity/URL filtering + report button + trust-weighting (account age/stake). Store-pull-level risk without it. **Done, phase 4b July 2026** — server-authoritative filter + trust-weighting + report-count suppression shipped (`src/lib/moderation.ts`, `reports` namespace, `/api/moderation/report`); see §3.2 done-note.
...
- 🟡 **A4 — Pale Coin anti-mint:** depth income becomes concave and clear-weighted (most on escape, little on bail), server-gated for online accounts, per-day caps; Coin-Bound bonus funded strictly from burned-stake pool (population-net-negative), never minted. First-clears/milestones go through a server unlock ledger. **Partial, phase 3b July 2026** — `computeCoinEarn` (concave, clear/first-clear weighted) and `computeCoinStakeSettlement` (pool-funded, pool-capped bonus, never negative) shipped server-side, called from the death/victory routes (not client-computed); see §9.0 done-note. **Not built:** explicit per-day earn caps and a dedicated server unlock ledger for first-clears/milestones — first-clear/milestone grants currently flow through the same per-run settlement path rather than a separate cap-enforcing ledger.
- 🟡 **A5 — Community-layer robustness:** aggregate over server-receipted deaths only; distinct-account counts and medians, not raw sums; per-account caps. Verified tips only (on-chain transfer before DB write). **Partial, phase 4a July 2026** — server-receipted-only ingestion, distinct-account thresholds (not sums), per-account caps, and a trailing-24h + future-clamp window all shipped in `/api/game/shift`/`world-shift-agg.ts`; see §3.2 done-note. **Residual:** distinct-account keying falls back to `walletAddress` when `authId` is absent, which is sybil-vulnerable where wallets are cheap (devnet) — must harden (prefer `authId`-only or a stake gate) before mainnet.
...
- ✅ **F3 — One new player verb (amends §6 scope):** a Bait/Feint action so enemy signature rules have answers beyond "Strike, but time it." Smallest addition that makes ~6 rules bloom. **Built, phase 2b July 2026** — `onBait`/`BaitResult` in `creature-rules.ts`, `baitCost` setting; see §6 done-note.
- ✅ **F4 — Authoring budget (amends §4.1):** ~12 signature nodes per zone + ~15 shared tagged transit beats recombined per zone — not 35 bespoke nodes × 5 zones × 6 locales. **Built, phase 2b July 2026** — all 5 zones shipped as 20-23 node branching graphs (not 35); side chambers landed within that budget rather than as bespoke bloat; see §4.1 done-note.
...
- ✅ **F6 — Binding streak (amends §9.0):** consecutive Coin-Bound escapes build a visible seal tier; a coin-bound death resets it. Makes soft-currency loss non-refungible (coins alone re-earn in ~3 runs = no sting). **Built, phase 3b July 2026** — `nextStreak`/`sealTier` shipped (tiers at streak 3/7/15); see §9.0 done-note.
