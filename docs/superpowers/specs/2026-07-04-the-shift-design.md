...
**Remaining for 2b:** bespoke signature-node prose (the ~12/zone authoring budget above), side chambers (dead-end detour nodes with item gates), and daily edges (open/close subset per shift) are not yet built — phase 2a shipped the graph structure, traversal, and depth projection that these depend on, not the authored content itself.
