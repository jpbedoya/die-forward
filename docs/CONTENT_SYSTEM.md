...
**Two synced copies:** zone packages live in two places — `mobile/lib/zones/<zone>.json` (canonical; bundled by the Expo app for graph-based dungeon generation) and `zones/<zone>.json` at the repo root (bundled by the Next.js web/API app via `src/lib/content.ts` `loadZone`, used by `session/start`, `agent/start`, and the 4a apex-creature allowlist in `game/shift`). They are kept **byte-identical** and a guard test (`src/lib/__tests__/zone-sync.test.ts`) fails CI on any drift. To edit a zone, change the `mobile/lib/zones/` copy and re-sync: `cp mobile/lib/zones/<zone>.json zones/<zone>.json`.
